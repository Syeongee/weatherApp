$(document).ready(function(){
        ...
        <li>
            <div class="top">
                ...
                <p class="interTemp">
                    ...
                    최고<span class="temp_max"> 15</span>℃</p>
                <h4>Cloud</h4>
                <p class="location"><span class="city"></span>, 
                <span class="nation"></span></p>
                </div>
            </div>
            <div class="bottom">
                <div class="wind">
    ...
    function w_info(){
        ...
        //첫번째 데이터가 존재한다면 두번째 버튼을 클릭했을 떄, (기존 데이터인 한개의 박스가 있는 것에 두개의 박스가 추가된다.)
        ...
        $("#weather li").each(function(index){
            ...
            $.ajax({
                url : `https://api.openweathermap.org/data/2.5/weather?q=${cityName[index]}&appid=f18aae9d2e96430c6161798042aac635`,
                dataType : "json",
                success : function(data){
                    ...
                    $("#weather li").eq(index).find(".temp_min").text(max_temp);
    ...
    function searching(){
        ...
        if(search_val.trim() == ""){ //검색어의 공백을 제거해보니 값이 없음(최소 공백만 넣은 상태)
            alert("검색어를 입력바랍니다.");
            $("#search_box").focus();
        }else{
    ...
    navigator.geolocation.getCurrentPosition(function(position){
        ...
        $.ajax({
            url : `http://api.openweathermap.org/data/2.5/weather?lat=${latitude}&lon=${longtitude}&appid=f18aae9d2e96430c6161798042aac635`,
            dataType : "json",
            success : function(data){
                const city = data.name;
                console.log("도시명 : " + city );
